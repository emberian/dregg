...
- **Wallet effects-per-turn.** No explicit `max_effects_per_turn` constant found in `sdk/`, `intent/`, or `turn/`. The implicit cap is `EffectVmAir::new(max_effects)`, which must be a power of 2 — current code paths construct AIRs with heights up to a few hundred rows.
