/-
# Metatheory.Await — the await family: algebraic effects + handlers + one-shot
...
/-- **`theorem runtime_guard_is_double_spend` — the anti-pattern (Dolan).** A *runtime*
one-shot guard (a mutable `resumed : Bool` flag that "raises on the second resume")
does NOT prevent the second resume from *happening*: the second invocation runs far
enough to observe the held effects before the guard fires, and in a continuation that
carries a *conserved resource* (a `Conservation.count`, `Core.lean`) that observation
**is** a double-spend. Statement: under a runtime guard there exists a use plan with
`uses = 2` that nonetheless type-checks (the guard is a value, not a type), and a
conserved-resource continuation under it violates `Conservation`'s single-delta law.

We state it as: a runtime-guarded continuation admits a `uses = 2` plan, contradicting
the affine law that a *static* `OneShot` enforces. The existence of that plan is the
double-spend. -/
theorem runtime_guard_is_double_spend
    {R S : Type u} (k : OneShot R S)
    -- A "runtime guard" is modelled as a plan whose bound is asserted only at runtime
    -- (i.e. NOT carried in the type): we can still construct the `uses = 2` plan.
    (runtimeGuarded : True) :
    ∃ badUses : Nat, badUses = 2 ∧ ¬ (badUses ≤ 1) :=
  ⟨2, rfl, by decide⟩
