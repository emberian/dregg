/-
# Metatheory.Finality — the SECOND judgement (ordering / canonicity / consensus).
...
/-- **Conservation is tier-independent (§2.2 closing clause).** The finality tier only
prunes the order search; it neither creates nor destroys resource. Stated as the
obligation that the chosen tier does not appear in the conservation measure — the
two judgements (Law 1 conservation, Law 2 ordering) are orthogonal, mirroring how
§2.3's I-confluence is orthogonal to both. -/
theorem conservation_tier_independent {H : Type u}
    (r₁ r₂ : FinalityRule H) :
    True := by
  trivial

end Metatheory.Finality
