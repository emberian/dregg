/-
# Metatheory.Liveness — GC-as-cell-liveness, and distributed cycle collection.
...
It is the **operational dual** of `Metatheory.Boundary`: where `Boundary` says a sound
...
Literature anchors (see `study-gc.md`):
  ...
  * **Refcounting ≠ reachability** (classic GC folklore; Bacon–Rajan trial-deletion) —
    ...
    refcounts ≥ 1 forever.
  * **The verify/find seam** (`Metatheory.Laws`) — *reachable* is semi-decidable
...
/-- **`theorem revocation_needs_consensus` — revocation (unlike collection) requires
agreement.** A revocation is *sound* (does not leave two vats disagreeing on whether a
cap is still authoritative) ONLY if all relevant parties reach `Consensus` on the
epoch-advance. Stated as: a revocation that is honest/effective across `parties` entails
`Consensus parties d`. This is the dual of `gc_safety_local`: GC is the positive
lifecycle (collect-when-unwanted, no consensus); revocation is the negative lifecycle
(kill-while-wanted, consensus-bound). `[study-gc.md §2, dregg2 §3]` -/
theorem revocation_needs_consensus
    (parties : List VatId) (d : RevocationDecision)
    (heffective : ∀ v ∈ parties, d.agreeing v → True) :
    Consensus parties d := by
  -- PRIMITIVE: the `heffective` antecedent is a placeholder (its conclusion is `True`, so
  -- it carries no information about `d.agreeing`); the real obligation — that an effective
  -- cross-vat revocation entails every party agreed the epoch advanced — is an axiom the
  -- operational consensus model must discharge, not derivable from this stub hypothesis.
  sorry
