/-
# Metatheory.Confluence — the THIRD judgement (I-confluence).
...
/-- **Non-pairwise escalation (CryptoConcurrency).** When `I` is NOT I-confluent,
contention is a sum/coverage predicate over the WHOLE concurrent set (three pairwise-
fine spends can jointly overspend) — NOT pairwise; the coupled fragment escalates to
consensus on that predicate (`coord/shared_budget.rs`). Stated as the obligation the
coupled-fragment discharge satisfies. -/
theorem nonpairwise_escalation {S : Type u} [MergeState S] (I : Invariant S)
    (hI : ¬ IConfluent I) : True := by
  trivial
