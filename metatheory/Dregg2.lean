...
import Dregg2.Exec.CellRuntime        -- the Robigalia "OS, not chain" payoff as THEOREMS (cand-A §5): checkpoint_restore_roundtrip + replay_deterministic_run + replayFrom_conserves + time_travel_fork (two admissible suffixes from one snapshot, both sound/conserving, diverging) over BOTH the ℤ-ledger cell and the name-keyed record cell (recTimeTravel_fork); reuses Cell.Snapshot/restore/replayFrom + recReplay_preserves_sumEquals (PROVED; #assert_axioms-clean; #eval non-vacuity)
...
-- ─── The replacement's reach to the wire + last-wave improvements ───
import Dregg2.Exec.CircuitEmit         -- the EXTRACTION EMITTER (the cascade's circuit, to the backends): emit (ConstraintSystem→wire) + emit_faithful (wire denotes the same system) + emittedKernel_bridge (emitted kernelCircuit ↔ fullStepInv); fingerprint-bound to the real Rust AIR via dregg-lean-ffi (differential PASS)
import Dregg2.Exec.CapTPGC             -- CapTP distributed GC by LEASE (closes CapTP §4 OPEN): captp_gc_by_lease + captp_no_premature_reclaim + captp_cycle_leak_is_the_price (reuses Liveness.dead_undecidable/crossvat_cycle_leaks — the impossibility, not faked)
import Dregg2.CatalogEffects           -- Phase (ii) completion: all 52 dregg1 Effect variants colored onto Spec.Conservation LinearityClass + per-class obligations + bespoke Regime discriminator; 69 thms #assert_namespace_axioms-clean
import Dregg2.Exec.TurnExecutor        -- the cascade REPLACEMENT turn-executor: Turn = call-forest of catalog-typed Actions run as an all-or-nothing transaction; execTurn_attests = all 4 StepInv conjuncts over the whole multi-Action turn (step-complete BY CONSTRUCTION) + execTurn_conserves/_balance_domain/_unauthorized_fails
import Dregg2.Exec.TurnForest          -- the NESTED call-FOREST (closes TurnExecutor's forest OPEN): execForest = recursive all-or-nothing executor over a tree of delegated Actions; execForest_no_amplify (Granovetter across the forest via derive_no_amplify) + execForest_conserves (N-ary CG-5, Σ=0 whole tree) + execForest_attests (4 StepInv conjuncts); cross-cell nested forest 1 OPEN
import Dregg2.Paco                      -- VENDORED + PORTED from hxrts/paco-lean (MIT, 4.26→4.30): parametrized coinduction (paco/upaco/gpaco/gupaco + companion/up-to + the `pcofix` tactic), 23 modules green. Powers CoinductiveAdversary's up-to-context closure (the coinductive-general-case OPEN, now CLOSED via obsBisim_of_uptoComm)
...
-- ─── Widening wave (2026-05-31): full op-set, cross-cell forest, runtime-character, eDSL-C, migration, all §8 gadgets to wire ───
import Dregg2.Exec.TurnExecutorFull    -- FULL op-set replacement executor: FullAction = balance | delegate/revoke | mint/burn, one execFull; execFull_attests = StepInv per kind across EVERY turn kind; execFullTurn_ledger transaction-level conservation
import Dregg2.Exec.CrossCellForest     -- the CROSS-CELL nested forest (closes TurnForest cross-cell OPEN): crossForest_no_amplify + crossForest_conserves (N-ary cross-cell Σ=0 binding-carried) + crossForest_attests; bilateral = Fin-2 slice; overlapping-cells 1 OPEN
import Dregg2.Exec.FullForest          -- the TREE-SHAPED FullActionA call-forest (wholesale-swap KEYSTONE): execFullForestA = recursive all-or-nothing tree executor over the FULL per-asset op-set, proved = execFullTurnA over the pre-order lowering (execFullForestA_eq_execFullTurnA); execFullForestA_conserves_per_asset = the FILL-1 CONSERVATION VECTOR end-to-end (NOT scalar); execFullForestA_no_amplify (Granovetter) + execFullForestA_each_attests (fullActionInvA per node); sameTargetForest = DelegationMode::None, cross-target routed to CrossCellForest
import Dregg2.Exec.CellRuntime         -- the Robigalia OS payoff as THEOREMS: checkpoint_restore_roundtrip + replay_deterministic_run + replayFrom_conserves + time_travel_fork (both branches sound); finite-run forms, νF fork noted
...
import Dregg2.Exec.CircuitEmitGadgets  -- emits the OTHER §8 gadgets to the wire: emittedTemporal/NonMembership/Pedersen/Dfa_bridge (each composes emit-faithfulness with the gadget's *_bridge)
import Dregg2.DSLEffect                -- DSL-C: the `dregg_effect <name> : <Class>` effects eDSL → Spec.Conservation LinearityClass coloring + inherited obligation (completes the eDSL trilogy; #assert_namespace_axioms-clean)
...
import Dregg2.Proof.BeaconSpaceInterior -- the CANONICAL interior-h non-vacuity witness: Measure.infinitePi (Bernoulli 3/4)^ℕ at strictly-interior h=3/4, indep_block discharged via real cross-view independence (infinitePi_pi); supersedes the Dirac h=1 boundary witness — the BeaconSpace abstraction is non-vacuously instantiable at a genuine interior honest-fraction
import Dregg2.Exec.ProofForest         -- WAVE 10: proof-carrying forest composition — per-node StepProofValid (§8 seam) × Linked (prev.newCommit = next.oldCommit) ⇒ whole-run StepInv via execForest_attests; the ship-the-tree architecture, aggregation deferred to perf
import Dregg2.Exec.TriDomain            -- E1: tri-domain conservation (balance + authority + metadata) — triConserved_of_execFull; the 3 domains dregg1's atomic.rs enforces, conserve independently (was balance-only)
import Dregg2.Exec.AuthModes           -- E2: the 6 authorization modes (OneOf/Custom/CapTpDelivered/Bearer/Token/Unchecked) with witness dispatch + soundness; captp_granted_le_held proves the non-amplification dregg1's Rust was missing
import Dregg2.Exec.EffectTransfer      -- E3 vertical-slice REFERENCE TEMPLATE: Transfer executable semantics → conserves → authorized → metadata → forward-sim (AbsStep); the pattern effects #2..51 instantiate
import Dregg2.Spike.TransferAirSoundness -- proof-of-method: the REAL air.rs:473 Transfer constraint over BabyBear, field-constraint+range ⇒ ℤ balance update, AND transfer_underflow_attack (the off-circuit wrap gap as a THEOREM — now closed in-circuit by the RANGECHECK Rust lane)
import Dregg2.Exec.EffectsPaired       -- E3-breadth: the Conservative/Paired effects (escrow/notes/obligations/queues/bridge-Σ=0) via the EffectTransfer template, factored into a generic pairedStep spine
import Dregg2.Exec.EffectsSupply       -- E3-breadth: the Generative/disclosed-supply effects (CreateCell/Factory/Spawn/BridgeMint/Lock/Cancel/Finalize); foreign-finality §8-portal
import Dregg2.Exec.EffectsAuthority    -- E3-breadth: the authority-edit effects (Introduce/Attenuate/Exercise/ValidateHandoff/Refresh/Drop/SetPermissions/RevokeDelegation), each with non-amplification
import Dregg2.Exec.EffectsState        -- E3-breadth: the Neutral/Monotonic/Terminal field+lifecycle effects (setField/seal/sovereign/emit/refusal/archive/destroy/refs…) via the generic field-write + non-interference
import Dregg2.Spec.ExecRefinementFull  -- E5: the GENERAL forward-sim refinement — AbsStep LTS + exec_full_refines_spec (every execFull step is a permitted abstract step) + the full operational square; whole-history closure isolated as named OPEN (not a sorry)
import Dregg2.Exec.ConditionalTurn     -- E4: executable conditional/await turns — execConditionalTurn (topo-sort + EventualRef slots) + condTurn_conserves/_atomic/_dependency_sound/_forward_sim; EventualRef read ↔ Await.Op.await
import Dregg2.Spike.EffectVmConstraints -- MAGNESIUM (circuit covers execution): 7 more REAL EffectVmAir constraints formalized in Lean — selector exactly-one, NoOp identity, transfer hi/dir, balance-lo range SOUNDNESS, nonce tick; underflow_now_impossible proves the RANGECHECK in-circuit range proof CLOSES the wrap gap the Transfer spike exposed
import Dregg2.Proof.CordialMiners       -- MAGNESIUM (real consensus): models dregg1's ACTUAL DAG-BFT (Cordial-Miners wave/leader/ratify/super-ratify commit rule) + cordial_agreement (no two conflicting committed leaders) by transferring BFT.honest_witness_in_intersection; SuperRatification now DERIVED from the lace (cordial_agreement_from_lace); liveness/GST/dissemination/Stingray named OPENs
import Dregg2.Spike.EffectVmConstraints2 -- MAGNESIUM (circuit covers execution, more lanes): SetField per-field gating + balance_hi range-soundness + state_commitment binding-shape (hash as §8 portal); exposes setfield_aux_honesty_gap (SetField intended-value rests on an off-circuit free aux column — a new gap, as a theorem)
import Dregg2.Authority.CaveatChain      -- CARRY-FORWARD (caveat face): the REAL macaroon HMAC append-only chain (not Ctx→Bool) — verify_iff_wellTagged + append_narrows + integrity_tail_binds + forgery_requires_mac_query (teeth, reduces forge⇒break-HMAC) + removal_breaks_tail; HMAC = §8 MacKernel portal
import Dregg2.Authority.ThirdPartyDischarge -- CARRY-FORWARD (caveat face): the REAL 3P discharge protocol (not a Bool flip) — accepts_iff (ticket/VID key-recovery ∧ bind-to-parent ∧ freshness ∧ predicate) + honest_discharge_accepted + stale/unbound/cross_bound rejection teeth; AEAD = §8 portal
import Dregg2.Authority.SelectiveDisclosure -- CARRY-FORWARD (caveat face): hidden-attribute predicate proofs (Gte/Lte/InRange) + selective reveal + anonymous unlinkable multi-show wired to credentials; hiding via the Privacy view-model, ZK soundness = §8 portal
import Dregg2.Authority.DesignatedVerifier -- CARRY-FORWARD (attestation face, THE REPUDIATION FIX): verifier-indexed DischargedFor + the transferability DIAL (public = ∀V ⇒ non-repudiable; designated = V₀-only with verifier-secret ⇒ deniable via the simulator property); DV-ZK = §8 portal
import Dregg2.Consistency               -- THE SOUNDNESS WITNESS (answers 'are the Prop-carriers hiding vacuity/contradiction'): dregg_consistent_nonempty = a single axiom-clean SystemModel jointly + discriminatingly instantiating all 11 system-level Prop-carriers; no carrier-pair derives False; the 3 sorries counterparted (conservation_step REALIZED, search_sound re-Verified, phi_functorial_concrete); NOT vacuous, NOT contradictory at the system level
import Dregg2.HandlerTransformer        -- DISCOVERY (higher-order handler-transformer): SafeStep preorder + instSafeStepFpu (the camera Fpu IS a safe-composition instance) + safe_transformer_composes + conservation_is_safe_transformer + overshare_rejected (TEETH: an unsafe transformer is genuinely refused) — a REAL first-order unification of 'safe handler-transformer' with 'frame-preserving update' (= the Iris handler frame-rule, de Vilhena–Pottier POPL'21); the Fpu=gluing weld + the comodel-morphism/sheaf-of-handlers tier are honest OPENs
