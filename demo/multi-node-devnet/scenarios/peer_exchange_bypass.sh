...
EOF
    ...
    # Same xid regardless of which federation runs it (the cells are
    # the only inputs).
...
if [ "$cells_before" = "$cells_after" ]; then
    ...
    # Solo-mode nodes can advance height autonomously; this isn't a
    # failure per se. Capture for diagnostic.
    devnet_warn "F1 cells changed during idle (solo-mode autoadvance)"
    record F1_ledger_unchanged_by_idle_observation true
fi
