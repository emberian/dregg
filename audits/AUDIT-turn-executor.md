...
The Effect VM bridging (`convert_turn_effects_to_vm`) also truncates 32-byte hashes to a single BabyBear via `% BABYBEAR_P` on the first 4 bytes — the same pattern flagged in AUDIT-wallet.md P2-5, and bilaterally consistent (executor and wallet both truncate), but this means many distinct turns map to the same effects-hash PI, so the proof "binds" to a coarse equivalence class rather than the turn.
...
**P1-1 — `verify_and_commit_proof` does not validate `execution_proof_new_commitment` is bound to anything the wallet signed.** (`turn/src/executor.rs:953` + `compute_turn_bytes` per AUDIT-wallet P2-10). The new commitment comes from `turn.execution_proof_new_commitment`. The executor confirms it equals the PI in the proof, but `Turn::hash()` (turn.rs:132-164) does NOT include `execution_proof`, `execution_proof_cell`, or `execution_proof_new_commitment`. An attacker who intercepts a signed Turn in flight can swap the proof and the new commitment for a different, valid (proof, new_commitment) pair (proof is unauthenticated; only its internal PI structure is checked) — the executor will accept it because the turn hash and signature still verify against the old fields. **Fix**: include execution_proof/_cell/_new_commitment in `Turn::hash()` (bump version tag to `pyana-turn-v3:`); update the wallet's `compute_turn_bytes` (AUDIT-wallet P2-10) to match.

**P1-2 — `convert_turn_effects_to_vm` truncates every 32-byte hash to its first 4 bytes via `% BABYBEAR_P`.** (`turn/src/executor.rs:1266-1273`). Both `hash_to_bb` and `field_element_to_bb` read `u32::from_le_bytes(h[0..4])` and reduce mod `BABYBEAR_P` (≈31 bits). All `Effect::NoteSpend.nullifier`, `NoteCreate.commitment`, `QueueEnqueue.message_hash`, `SetField.value`, `pipeline_id`, etc. collapse to 31-bit identifiers in the circuit PIs. The wallet's `convert_effects_to_vm` is identical (AUDIT-wallet P2-5), so executor and wallet *agree* — but the proof is bound to a small equivalence class rather than the turn. An attacker can construct two distinct turns whose circuit views are identical (collisions are trivially findable). Net effect: the proof verifies multiple turns simultaneously, so the wallet/executor must still rely on the *un-shrunk* effects (via `compute_effects_hash`) for binding. Since `compute_effects_hash` is also a BLAKE3 of the original effects, the binding survives — but the proof PIs become decorative for any cross-checking that uses *only* the PI values. **Fix**: use `bytes32_to_babybear` (the 8-element encoding) for all `hash_to_bb` callers, and widen the circuit PI layout accordingly.
...
**P2-1 — `Turn::hash()` excludes `sovereign_witnesses`, `conservation_proof`, `custom_program_proofs`, `execution_proof*` fields.** (`turn.rs:132-164`). Same shape as AUDIT-wallet P2-10. An attacker with write-access to the in-flight SignedTurn can swap any of these without invalidating the signature. The agent's signed claim is only over agent/nonce/forest/fee/memo/valid_until/depends_on/previous_receipt_hash. **Fix**: extend `Turn::hash()` to cover all semantically-load-bearing fields; bump version tag.
...
**P3-1 — `compute_signing_message` and `compute_partial_signing_message` use `self.local_federation_id` at verification time.** If `set_local_federation_id` is called between when the wallet signs and when the executor verifies (e.g. due to operator config change), all in-flight turns become invalid. Not a security issue but a liveness footgun.
...
**Turn signature binding:** `compute_signing_message` (executor.rs:3292) and `compute_partial_signing_message` (3343) include `federation_id` and `turn_nonce`. The wallet's signing message is computed via `compute_turn_bytes` (AUDIT-wallet P2-10) which excludes `execution_proof*` fields — see P1-1 and P2-1.
...
4. **`previous_receipt_hash` enforcement (P0-3)** — do you want me to add a `last_receipt_hash: Option<[u8; 32]>` to `CellState` and check it in `execute()`? The wallet would also need to plumb this through `build_authorized_turn` and queue ops (currently hardcoded to `None`, see wallet P3-6).
