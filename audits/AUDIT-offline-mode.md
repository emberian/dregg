...
**Read-only audit.** Tracing call sites in `sdk/src/wallet.rs`, `sdk/src/runtime.rs`, `turn/src/executor.rs`, `turn/src/fast_path.rs`, `turn/src/turn.rs`, `turn/src/verify.rs`, `federation/src/solo.rs`, `federation/src/lib.rs`, `node/src/api.rs`, `node/src/blocklace_sync.rs`, `captp/src/store_forward.rs`, `captp/src/session.rs`, `sdk/src/captp_client.rs`.
...
- **Crypto/proof construction.** The `AgentWallet` and `AgentRuntime` paths that build, sign, prove and apply turns are **pure, synchronous, in-process functions.** They reach the network only via three explicitly `async + #[cfg(feature = "federation-client")]` methods that ship as opt-in REST clients.
...
- **Bridge.** The offline → online transition has **two distinct seams**:
  ...
  2. **Independent wallet → federation** (the wallet was never connected): the wallet just hangs on to its in-memory ledger and receipt chain. To make it externally visible, *some* online node must accept and re-execute the turn through the blocklace path (`node/src/api.rs::post_submit_turn` → `gossip_turn` → `execute_finalized_turn`).
- **The non-answer.** What pyana does **not** have today is a "pure offline wallet that accumulates receipts then atomically posts them to consensus." The wallet's offline receipts are *self-attested only*; nothing in the on-chain path consumes a wallet-generated `TurnReceipt`. The chain rebuilds its own receipt by re-executing the turn.

---

## 1. AgentWallet offline: signing, building, proving

**Result: a wallet can sign turns, build authorization proofs, and produce STARK proofs entirely offline. None of the canonical "build & sign" entry points are `async` or touch the network.**

### 1a. Wallet API surface — local vs network

In `sdk/src/wallet.rs`, network reachability is fully visible as Rust syntax. Searching for every method whose body talks to a peer:

```
$ grep -n "federation-client\|reqwest::" sdk/src/wallet.rs
...
**Exactly three** wallet methods make network calls, and all three are feature-gated:
- `register_with_federation` (wallet → `/cells/register`)
- `deregister_from_federation` (wallet → `/cells/deregister`)
- `deploy_program` (wallet → `/programs/deploy`)

Everything else is pure compute. The "core dance" methods we care about are all **synchronous (`pub fn`, not `pub async fn`)**:

| Method | Behaviour |
|---|---|
| `sign_turn` (wallet.rs:2361) | Hashes the turn locally, calls `signing_key.sign(&turn_bytes)`, returns a `SignedTurn`. |
| `sign_action` (wallet.rs:2403) | Builds the canonical signing message via `TurnExecutor::compute_signing_message`, signs it, returns an `Action`. |
| `make_action` (wallet.rs:2440) | Constructs an unsigned `Action` and sends it through `sign_action`. |
| `make_turn` / `make_turn_for` (wallet.rs:2475, 2480) | Wraps an `Action` in a `CallTree`/`CallForest`/`Turn`. Reads `self.receipt_chain.last()` to populate `previous_receipt_hash`. |
| `build_authorized_turn` (wallet.rs:2548) | Builds an `AuthRequest`, calls `self.authorize(...)` → STARK proof bytes, wraps into a `Turn`, calls `sign_turn`. |
| `prove_authorization` (wallet.rs:2882) | Calls `pyana_bridge::BridgePresentationBuilder::prove(...)` — pure prover. |
| `prove_authorization_with_issuer_key` (wallet.rs:2957) | Same, with out-of-band issuer key for attenuated tokens. |
...
3. `self.wallet.read()...sign_bytes(&message)` — local signing.
4. Reattach `Authorization::from_sig_bytes(sig.0)` to the action.
5. Acquire `self.ledger.lock()`.
6. Allocate a nonce from `self.nonce`.
7. Read `self.wallet.read()...receipt_head().map(|r| r.receipt_hash())` to populate `previous_receipt_hash`.
...
10. On `TurnResult::Committed { receipt, .. }`, `self.wallet.write()...append_receipt(receipt.clone())`.

**External calls: zero.** No `await`, no socket, no DNS, no peer query. The agent's wallet, its 1M computron starting balance, the executor, the ledger, and the receipt chain are all in one process.

### 1c. Sub-agent / domain-separated wallets

`AgentWallet::derive_sub_agent(index)` (wallet.rs:1024) performs HKDF-style derivation from the parent's secret key bytes. Pure compute. The result is an independent `AgentWallet` whose receipt chain starts at zero.
...
- `finality` defaults to `Final` (turn.rs:355-358). The runtime's executor leaves it at `Final`. **The wallet's offline-produced receipts claim full finality.**
...
**So an offline `AgentWallet`'s receipt chain is a self-attested record that:**
...
The wallet's `AgentRuntime` path does not know about federation modes and produces `Final` receipts unconditionally. This is arguably a bug — a wallet that's never been online cannot honestly stamp `Final` — but it's the current code.
...
`AgentWallet::reset_receipt_chain()` doesn't exist as a public method, but `append_receipt` does (wallet.rs:1775). Alice can:
...
Nothing in `sdk/src/wallet.rs` or `turn/src/executor.rs` detects this — the executor's `last_receipt_hash` map (executor.rs:594-607) only enforces forward consistency per `(executor, agent)` pair. Two separate `TurnExecutor` instances cannot see each other's history. (One can be the local in-memory runtime executor; another can be the node's executor.)
...
**The peer's view of "what really happened" is the result of re-executing the turn against the same deterministic executor — not the receipt the originating wallet stored.** Wallet-produced receipts never cross the wire as canonical evidence; the turn does, and consensus generates the canonical receipt at the destination.
...
### 4c. The wallet's "I was offline, here are my turns" path

There is no batch-sync method on `AgentWallet`. The wallet does not expose `replay_chain_to_node`, `submit_all_pending`, or similar. The intended workflow today appears to be:

- Wallet builds + signs + (optionally) proves each turn offline.
- Wallet keeps the `SignedTurn` blobs (in memory; persistence is the caller's job).
- When the wallet is online again, the caller submits each `SignedTurn` to a node's HTTP `/turn/submit` endpoint one by one.
...
Note that `previous_receipt_hash` is populated by the wallet from its own local chain, but the node's executor (`turn/src/executor.rs:843-864 check_previous_receipt_hash`) compares it against the node's `last_receipt_hash` map. **If the offline wallet's chain has receipts the node has never seen, the node will reject the next turn with `TurnError::ReceiptChainMismatch`.** This is `EXECUTOR-HONESTY-AUDIT.md` Stage 9 R-4 territory.

This is the fundamental seam:

```
Wallet's local ledger                Node's ledger (consensus-derived)
       |                                       |
   ...
   chain head = h_offline                  chain head = h_online (older)
       ...
       +-------- submit turns ---------------->|
                  ...
                  match the wallet's h_offline |
```

**The wallet's locally-built receipt chain is not the chain that ends up on consensus.** They share `turn_hash`, `pre_state_hash`, `post_state_hash` *if* the underlying state genuinely agrees, but the receipts themselves are different objects (different `timestamp`, different `executor_signature`, possibly different `federation_id`).
...
Critical: `proposer_cell` and `treasury_cell` are `Option<CellId>` (executor.rs:542-544). If `None`, the corresponding share is burned. The wallet's `AgentRuntime` never sets either (`runtime.rs:123` constructs the executor with `TurnExecutor::new(...)` and stops). **Offline, 100% of the fee is burned.** That's not a bug per se — there's no federation to pay — but it means the wallet's local ledger drains by `turn.fee` per turn against itself only.
...
`turn/src/economics.rs::EpochMinter::maybe_mint` (line 205) credits the **treasury cell** when an epoch boundary is crossed. The treasury cell is a federation-configured cell (executor.rs:893). Offline, no minting happens (and no treasury to mint to). Long-term offline-only operation in a wallet leads to a deflationary drain to zero.

### 5e. The fee=10_000 demo number

`sdk/src/runtime.rs:276` hardcodes `fee: 10_000` in `AgentRuntime::execute`. The two-AI handoff demo runs through this path; the demo also funds Alice and Bob with `1_000_000` (`runtime.rs:117`). 100 turns = empty wallet.
...
In `sdk/src/wallet.rs:5506-5800`, the wallet's queue methods are all `pub fn` (not async), and each returns a `Turn` ready for submission. The wallet builds the turn locally; no network. The `previous_receipt_hash` is set from `self.receipt_chain.last()`.

The actual queue state lives in `TurnExecutor::queue_program_registry` (executor.rs:593) and is mutated when `apply_effect` processes `Effect::QueueAllocate / QueueEnqueue / QueueDequeue / QueueAtomicTx`. Because the registry hangs off the executor instance, **each executor has its own queue state.** The wallet's `AgentRuntime` executor has a separate queue registry from the node's executor.
...
Queue programs (`Cargo.toml` shows `queue_programs.rs` in `turn/src/`) bind validation programs to queue IDs by VK hash. The VK hash itself is computed offline; deploying it to the federation requires `wallet.deploy_program(node_url, ...)` (wallet.rs:5267, network-only).
...
| Create a sovereign cell (`make_sovereign`) | yes | requires `register_with_federation` to be online-visible | wallet.rs:4269. |
...
`LiveRef::pipeline` (line 162) is identical (a comment confirms). **So today, the wallet's CapTP path silently drops outbound messages.** The store-and-forward primitives in `captp/src/store_forward.rs` exist, but the wallet's high-level send/pipeline API doesn't yet call them. The integration seam is unfinished.
...
1. **Solo-mode `Tentative → Final` promotion path is not visible.** The protocol docstring in `federation/src/solo.rs:14-20` says tentative receipts are promoted to Final on rejoin. I cannot find the code that walks the wallet's receipt chain, sees a `Tentative` flag, validates against the merged nullifier log, and rewrites finality. (Search: `Finality::Final` outside `executor.rs`; nothing.) Is this scheduled work, or am I missing it?

2. **`AgentRuntime` stamps `Final` on offline receipts.** `sdk/src/runtime.rs::AgentRuntime` constructs `TurnExecutor::new(...)` without setting an executor signing key or downgrading finality. Receipts produced offline therefore claim `Finality::Final` and ship with `executor_signature: None`. Is this intentional (the wallet trusts itself unconditionally) or should `AgentRuntime` be aware that it's an unattested local executor and stamp `Finality::Tentative`?

3. **Wallet receipt chain vs. consensus receipt chain divergence.** When an offline wallet builds receipts R1..R10 and then submits the underlying turns to a node, the node's executor produces its own receipts R1'..R10' with different timestamps, federation_ids, and executor signatures. The wallet's `previous_receipt_hash` populated from R10 will not match the node's `R10'.receipt_hash()` (different bytes). What's the contract for reconciling these? Should the wallet (a) tear down its local chain on first successful node submit and rebuild from server, (b) maintain both chains and use the server's for `previous_receipt_hash`, or (c) something else? `EXECUTOR-HONESTY-AUDIT.md` and `WITNESSED-RECEIPT-CHAIN-DESIGN.md` may already answer this; the in-code answer isn't obvious from the wallet/runtime alone.

4. **Wallet-side batch submit.** Is there an intended "drain my offline turns to the federation" workflow? Today the wallet has no such method; callers must submit one-by-one. If the wallet has accumulated dozens of locally-applied turns, the all-or-nothing semantics of submitting them is ambiguous (what happens if turn 17 of 30 is rejected at the node?).

5. **`local_federation_id` in the offline executor.** `AgentRuntime::execute` calls `TurnExecutor::compute_signing_message(&action_unsigned, &self.executor.local_federation_id)` — but `runtime.rs:123` creates the executor with `TurnExecutor::new(...)`, which sets `local_federation_id: [0u8; 32]` (executor.rs:635). So the wallet signs every action as if it lives in federation `0x00..00`. If that wallet then submits to a real federation, the action signature will not validate against the node's `local_federation_id`. (The cross-federation replay defense in receipt.rs:374-378 is intentional, but it cuts both ways.) Is there an intended "set federation id on AgentRuntime" hook?
...
7. **Queue state divergence.** Offline-allocated queues live in the wallet's local `TurnExecutor::queue_program_registry`. The node has no awareness of them until the underlying `Effect::QueueAllocate` turn is submitted. Until then, anything that depends on a queue ID derived from that allocation has no chance of validating elsewhere. Is there a doc-recommended "always allocate queues online first" workflow, or is the assumption that anyone using queues is doing so within an online runtime?
...
9. **Wallet receipt-chain reset on conflict.** If the wallet's chain head H_offline disagrees with the node's H_online for the same agent, every subsequent wallet-built turn is structurally invalid at the node (`ReceiptChainMismatch`). There's no public `AgentWallet::reset_receipt_chain()` method. Is there an intended recovery path other than "destroy and rebuild the wallet"?

10. **Equivocation cost.** The blocklace auto-evicts equivocators (`blocklace_sync.rs:803-823`) with `constitution.auto_evict(&proof)`. The wallet can also auto-equivocate inadvertently by submitting two divergent chains to two different nodes (if it ever had two `TurnExecutor` instances active). Should the wallet itself enforce that it never produces two distinct receipts at the same `(agent, nonce)`? Today it doesn't.
...
Primary call sites:
- `/Users/ember/dev/breadstuffs/sdk/src/wallet.rs` — wallet API, sign_action/sign_turn/build_authorized_turn/prove_*/queue ops; lines 2361, 2403, 2440, 2475, 2548, 2882, 5134, 5506.
...
- `/Users/ember/dev/breadstuffs/node/src/api.rs::post_submit_turn` line 1124 — entry point that runs the wallet's turn locally on the node then gossips.
